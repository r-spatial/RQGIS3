...
This provides the user with an extensive suite of GIS functions, since QGIS allows you to call native as well as third-party algorithms via its processing framework (see
...
Third-party providers include among others GDAL, GRASS GIS, and SAGA GIS. **RQGIS3** brings you this incredibly powerful geoprocessing environment to the R console.
...
Here, we look for a geoalgorithm that contain the word `centroid`.  
...
Here, we'll choose the QGIS function named `native:centroids`. 
...
In our case, `native:centroids` has only two function arguments and no default values. 
...
Of course, you can also use a path to specify `INPUT` (e.g. "ger.shp") which is the better option if your data is somewhere stored on your hard drive. 
