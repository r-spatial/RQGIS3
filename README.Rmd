...
Though **RQGIS3** works when running it in a plain R session in the terminal, it crashes the R session when ran inside RStudio (see https://github.com/r-spatial/RQGIS3/issues/10 and https://github.com/rstudio/rstudio/issues/4606).
...
Please refer also to the instructions on the [geocompr landing page](https://github.com/robinlovelace/geocompr#running-geocompr-code-in-docker) dehow to use the docker image.
...
Secondly, we would like to find a QGIS geoalgorithm that is able to compute the centroids of a polygon vector layer. 
...
**Calling the QGIS3 Python API**

Internally, `open_app()` first sets all necessary paths (among others the path to the QGIS Python binary) to run QGIS3, and secondly opens a QGIS3 application with the help of [reticulate](https://github.com/rstudio/reticulate). 
`open_app()` establishes a tunnel to the QGIS3 Pyton API.
Hence, it is run automatically by all **RQGIS3** functions that need access to the QGIS3 Python API.
...
Additionally, you can use your own Python commands to customize **RQGIS3** as you wish. 
